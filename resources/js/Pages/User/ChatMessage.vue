<template>
    <div class="page-wrapper">
        ...
        <el-card class="box-card ml-5 mr-5 mt-5 mb-5">
            <template #header>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-8 bg-white ">
                            ...
                            <div class="chat-message">
                                <ul class="chat">
                                    <li v-for="(message, index) in chatStore?.latestchat?.messages" :key="index"
...
const sendMessage = async () => {
    ...
    try {
        ...
        const response = await axios.post(`/send-chat-message/${store.authuser.id}`, {
            chatId: chartid.value == null ? chatStore?.latestchat?.id : chartid.value,
        ...
        if (response.status === 200) {
            ...
            chatStore?.latestchat?.messages.push(response.data.message);
            // chatStore?.adminlatestchat?.messages.push(response.data.message);
            // chatStore.adminlatestchat.messages.push(newMessage);

            chatStore?.adminchats.forEach((chat, index) => {
                if (chat.id == newChat.id) {
                    chatStore.adminchats[index].messages = newChat.messages;
                }
            })
        }
    } catch (error) {
...
const fetchChatData = async (productId, userId) => {
    chatStore.chats = null
    chatStore.latestchat = null
    console.log('here product')

    try {
        const response = await axios.get(`/get-chat-data/${productId}/${userId}`);
        if (response.status == 200) {
            product.value = response.data.product;
            user.value = response.data.user;
            chat.value = response.data.chat;
            messages.value = response.data.messages;
            chatStore.chats = response.data.chats
            chatStore.latestchat = response.data.currentChat
...
const fetchAllChatData = async () => {
    chatStore.chats = null
    console.log('here not product')
    // chatStore.latestchat = null
    let userId = store?.authuser?.id
    try {
        const response = await axios.get(`/get-all-chat-data/${userId}`);
        if (response.status == 200) {
            // product.value = response.data.product;
            // user.value = response.data.user;
            // chat.value = response.data.chat;
...
const handleIncomingMessage = (event) => {
    ...
    if (product && product.id === chatStore?.latestchat?.product.id) {
        chatStore?.latestchat?.messages.push(newMessage);
    }
};
...
onMounted(() => {
    ...
    if (productId != '' && productId != null && productId != undefined) {
        fetchChatData(productId, userId);
        console.log('empty string exist')
    }
    if (productId == '' && productId == null && productId == undefined) {
